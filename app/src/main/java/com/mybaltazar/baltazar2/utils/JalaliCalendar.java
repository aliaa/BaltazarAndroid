...
public class JalaliCalendar extends Calendar {
    ...
    public static String getDate(Calendar c) {
        YearMonthDate ymd = new YearMonthDate(c.get(YEAR), c.get(MONTH), c.get(DATE));
        ymd = gregorianToJalali(ymd);
        return ymd.toPersianString();
    }

    public static String getTime(Calendar c) {
        int x = c.get(HOUR_OF_DAY);
        String hh = x < 10 ? "0" + x : String.valueOf(x);
        x = c.get(MINUTE);
        String mm = x < 10 ? "0" + x : String.valueOf(x);
        return StringUtils.getPersianNumber(hh) + ":" + StringUtils.getPersianNumber(mm);
    }
    ...
    public static class YearMonthDate {
        ...
        public String toPersianString() {
            int year = getYear();
            if (year >= 1332 && year < 1400)
                year %= 100;
            return StringUtils.getPersianNumber(year)
                    + "/" + StringUtils.getPersianNumber(getMonth() + 1) + "/"
                    + StringUtils.getPersianNumber(getDate());
        }
    }
}
